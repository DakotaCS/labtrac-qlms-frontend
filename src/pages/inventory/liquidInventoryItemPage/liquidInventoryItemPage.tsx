 ...
 * @version 1.2
 * @description Liquid Inventory Item Page
 */
...
import { LiquidInventoryItem, Category, Location, Unit } from '../../../components/types';
import './liquidInventoryItemPage.css';
...
const LiquidChemicalInventoryPage: React.FC = () => {
  const [inventoryItems, setInventoryItems] = useState<LiquidInventoryItem[]>([]);
  ...
  const [selectedItem, setSelectedItem] = useState<LiquidInventoryItem | null>(null);
  ...
  const [addedItem, setAddedItem] = useState<LiquidInventoryItem | null>(null);
  ...
  const fetchInventoryItems = useCallback(async () => {
    try {
      ...
      const response = await apiClient.get('/inventory/liquid/pageable', {
        params,
      });
  ...
  const handleAddInventoryItem = async (data: any) => {
    try {
      const response = await apiClient.post('/inventory/liquid', data);
      ...
      const itemResponse = await apiClient.get(`/inventory/liquid/${newItemId}`);
  ...
  const handleUpdateQuantity = async (id: number, quantityUsed: number) => {
    try {
      await apiClient.patch(`/inventory/liquid/${id}/quantity`, { quantityUsed });
      fetchInventoryItems();
  ...
  const handleUpdateDetails = async (id: number, data: any) => {
    try {
      await apiClient.patch(`/inventory/liquid/${id}`, data);
      fetchInventoryItems();
  ...
  const handleDeleteItem = async (id: number) => {
    try {
      await apiClient.delete(`/inventory/liquid/${id}`);
      fetchInventoryItems();
  ...
  const openUpdateQuantityPopup = (item: LiquidInventoryItem) => {
    setSelectedItem(item);
    setShowUpdateQuantityPopup(true);
  };

  const openUpdateDetailsPopup = (item: LiquidInventoryItem) => {
    setSelectedItem(item);
    setShowUpdateDetailsPopup(true);
  };

  const viewDetailedInfo = (id: number) => {
    window.location.href = `/inventory/liquid/${id}`;
  };

  const handlePrintLabel = async (item: LiquidInventoryItem) => {
    try {
  ...
  return (
    <Layout>
      <div className={`liquid-inventory-page ${menuCollapsed ? 'collapsed' : ''}`}>
        <h1 className="page-title">Liquid Chemical Inventory</h1>
...
const InventoryForm: React.FC<InventoryFormProps> = ({
  ...
  const [quantityUnitCode, setQuantityUnitCode] = useState<string>(
    units[0]?.quantityUnitCode || ''
  );
...
export default LiquidChemicalInventoryPage;
