 ...
 * @version 1.2
 * @description Liquid Inventory Item Details Page
 */
...
import {LiquidInventoryItemDetails, Note, Category, Location, LiquidInventoryItem,} from '../../../../components/types';
import './liquidInventoryItemDetailsPage.css';

const InventoryItemDetailsPage: React.FC = () => {
  const { id } = useParams<{ id: string }>();
  const [itemDetails, setItemDetails] = useState<LiquidInventoryItemDetails | null>(null);
  ...
  const fetchItemDetails = useCallback(async () => {
    try {
      const response = await apiClient.get(`/inventory/liquid/${id}`);
  ...
  return (
    <Layout>
      <div className="inventory-item-details-page">
        ...
        <button
          className="note-and-notification-button"
          onClick={() => setShowUpdateNotificationsPopup(true)}
        >
